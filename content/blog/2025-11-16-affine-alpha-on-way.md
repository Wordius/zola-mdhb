+++
title = "Affine Alpha is on the way"
description = "Affine, a cross-platform, open-source markdown editor, is fast approaching its Alpha release"
date = 2025-11-16
[taxonomies]
tags = ["markdown", "Affine"]
+++

{{ image(path="/img/affine-alpha.webp", caption="Affine Alpha is soon to be released") }}

Affine describes itself as ‘a next-gen knowledge base and open-source alternative to Notion and Miro’, but access to the Alpha has yet to be released.
...
>
> By collating user feedback, we have also learnt that there is **a strong demand for dark mode**.

Markdown is quite easy to use, apparently:
...
The major drawback of using Affine – for me anyway – is it’s reliance on AI. But that can be disabled. If you trust it, that is. I tested it on a bunch of past notes that were useless to me and did not contain anything personal:

To disable the AI function, go to: 

- Settings > Editor > Affine AI and disable and reload

While you’re at it, go to `Settings > About  AFFINE` and disable `Telemetry`. This feature allows Affine to collect data on how you use the app. Might as well be on the safe side.